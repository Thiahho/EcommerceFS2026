...
export default function AdminVariantsPage() {
  ...
  useEffect(() => {
    ...
    const loadVariants = async () => {
      try {
        ...
        setStatus(error instanceof Error ? error.message : 'No se pudieron cargar las variantes.');
      }
    };
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editingId) {
        const updated = await adminFetch<AdminProductVariant>(`/api/admin/variants/${editingId}`, token, {
          method: 'PUT',
          body: JSON.stringify(form),
        });
        setVariants((prev) => prev.map((item) => (item.id === editingId ? updated : item)));
      ...
      setStatus(error instanceof Error ? error.message : 'No se pudo guardar la variante.');
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleDeactivate = async (variantId: string) => {
    ...
    try {
      await adminFetch(`/api/admin/variants/${variantId}`, token, { method: 'DELETE' });
      setVariants((prev) => prev.map((item) => (item.id === variantId ? { ...item, active: false } : item)));
      setStatus('Variante desactivada.');
    } catch (error) {
      setStatus(error instanceof Error ? error.message : 'No se pudo desactivar la variante.');
    }
  };
    ...
    <div className="space-y-8">
      ...
      <section className="card space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-slate-500">Seleccioná el producto para ver sus variantes.</p>
          </div>
          <select
      ...
      <section className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
        <div className="card space-y-4">
          <div>
            <h2 className="text-lg font-semibold text-ink">Listado de variantes</h2>
            <p className="text-xs text-slate-500">Mostramos variantes activas e inactivas.</p>
          </div>
            ...
            <table className="min-w-full text-sm">
              <thead className="text-left text-xs uppercase text-slate-400">
                <tr>
                  ...
                  <th className="py-2">Almacenamiento</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {variants.map((variant) => (
                  <tr key={variant.id} className="text-slate-600">
                    <td className="py-3 font-semibold text-ink">{variant.sku}</td>
                    <td className="py-3">{variant.color}</td>
                    <td className="py-3">{variant.ram}</td>
                    <td className="py-3">{variant.storage}</td>
                    <td className="py-3">
                      {variant.stockActual} / {variant.stockReserved}
                    </td>
                    <td className="py-3">
                      <span className={variant.active ? 'text-moss' : 'text-slate-400'}>
          ...
          <div className="grid gap-3 text-sm">
            ...
            <div className="grid grid-cols-2 gap-3">
              <input
                ...
                onChange={(event) => setForm((prev) => ({ ...prev, stockActual: Number(event.target.value) }))}
                className="rounded-2xl border border-slate-200 px-4 py-3"
              />
              <input
                type="number"
                ...
                onChange={(event) => setForm((prev) => ({ ...prev, stockReserved: Number(event.target.value) }))}
                className="rounded-2xl border border-slate-200 px-4 py-3"
              />
            </div>
