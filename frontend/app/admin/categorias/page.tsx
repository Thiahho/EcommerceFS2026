...
export default function AdminCategoriesPage() {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        const response = await adminFetch<AdminCategory[]>('/api/admin/categories', token);
        setCategories(response);
      } catch (error) {
        setStatus(error instanceof Error ? error.message : 'No se pudieron cargar las categorías.');
      }
    };
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editingId) {
        const updated = await adminFetch<AdminCategory>(`/api/admin/categories/${editingId}`, token, {
          method: 'PUT',
        ...
        const created = await adminFetch<AdminCategory>('/api/admin/categories', token, {
          method: 'POST',
          body: JSON.stringify(form),
        });
      ...
      setStatus(error instanceof Error ? error.message : 'No se pudo guardar la categoría.');
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleDeactivate = async (categoryId: string) => {
    ...
    try {
      await adminFetch(`/api/admin/categories/${categoryId}`, token, { method: 'DELETE' });
      setCategories((prev) => prev.map((item) => (item.id === categoryId ? { ...item, active: false } : item)));
      setStatus('Categoría desactivada.');
    } catch (error) {
      setStatus(error instanceof Error ? error.message : 'No se pudo desactivar la categoría.');
    }
  };
      ...
      <section className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
        <div className="card space-y-4">
          <div>
            ...
            <p className="text-xs text-slate-500">Categorías activas e inactivas.</p>
          </div>
              ...
              <tbody className="divide-y divide-slate-100">
                {categories.map((category) => (
                  <tr key={category.id} className="text-slate-600">
                    ...
                    <td className="py-3">
                      <span className={category.active ? 'text-moss' : 'text-slate-400'}>
        ...
        <div className="card space-y-4">
          <div>
            ...
            <p className="text-xs text-slate-500">Nombre y slug son obligatorios.</p>
          </div>
